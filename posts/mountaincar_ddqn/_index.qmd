...
description: "Solve OpenAI's MountainCar-v0 environment with a double deep Q network."
author: "John King"
date: "4/12/2020"
format:
  html:
...
The [DeepMind researchers](https://arxiv.org/abs/1509.06461) developed the Double Deep Q-Network after observing that the DQN target network is prone to overestimating Q-Values.
...
> [Aurélien Géron](https://www.amazon.com/Hands-Machine-Learning-Scikit-Learn-TensorFlow/dp/1492032646)
...
The only other change needed is to create the target model by cloning the online model as shown in the two lines commented with `# DDQN` below. That's it! So now I'll train the Dueling DQN agent for 2000 episodes and see how its performance compares to DQN.
...
Compared to the DQN, the best score is better, but it looks less stable. Eyeballing things, it looks like the average score over the last 200 episodes is 125-130-ish, which is better than the 145-ish with DQN. There are a lot of sub-110 scores, though, so this has some promise for pursuing the goal of "solving" the environment (100 successive episodes with 110 or fewer steps).
...
Seems to be a little more stable. Same best score, and maybe a slightly better average score over the last 200 episodes. Still haven't technically solved the environment, though. Looks like that's going to take some work.
